...
def get_container():
    image=data.DEFAULT_DOCKER_IMAGE
    if 'pass' not in request.args:
        print('no pass')
        if 'image' in request.args and request.args['image'] in data.DOCKER_IMAGES:
...
def del_all_containers():
    con=client.containers.list(filters={'ancestor':data.DOCKER_IMAGES})
    deleted_containers={}
    for i in con:
